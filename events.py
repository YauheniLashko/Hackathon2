...
def event(enemies, score, group_pearls, pearl, window):
...
def do_pause(window):
    ...
    while paused:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit()
            pause_text = pygame.font.SysFont('comicsansms', 50).render("ПАУЗА! Нажми пробел для продолжения", True,
                                                                       (209, 52, 52))
            window.blit(pause_text, (100, 400))
